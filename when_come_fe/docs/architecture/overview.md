...
- **도착정보 조회 범위 (2026-05-10~):** 모든 그룹(과거 포함)을 동시 조회 — past/future 통합
  ...
  - **isPast/isFuture 접힘/펼침 패턴 (2026-05-31~):** 접힘 = 풀너비 한 줄 바 (정류장명 / 로 join + 최단 도착시간 + ChevronDown 토글). 펼침 = isCurrent와 동일한 가로 2분할 큰 카드 + 노선별 도착정보 상세. `expandedSteps: Set<number>` (stepGroup index)로 토글 상태 관리. past는 바 + 펼침 모두 `opacity-60` 유지.
  - **isPast 시각 차이:** 바에 `CheckCircle2` 아이콘 + `opacity-60`. 펼침 영역은 isFuture와 동일한 노선별 도착 상세
  ...
  - **isCurrent 카드 좌우 swap (2026-05-31~):** stepGroup 2개일 때 두 카드를 dnd-kit으로 드래그하여 좌우 순서 교환. `SortableSegmentCard` + `PointerSensor activationConstraint: { delay: 500, tolerance: 5 }`로 모바일 우발 트리거 방어 (꾹 누르기 0.5초). **드래그 핸들 영역 = 카드 상단 헤더(정류장명+ARS, `border-b` 위쪽)로 한정** (2026-06-02~). 카드 전체에 listeners를 걸면 본문 위 세로 스크롤이 막혀 — `SortableSegmentCard`를 render-prop 패턴으로 전환해 `handleProps`를 자식에서 헤더 div에만 spread. 헤더에는 `touch-none cursor-grab` 적용, 본문(도착 정보 리스트)은 평소처럼 스크롤 가능. 같은 sensor 패턴이 Home 상단 경로 이름 칩(`chipSensors`)과 Favorites 카드(`Favorites.tsx`)에도 일괄 적용됨. swap 시 두 seg의 `sequence`만 교환해 `updateRoute(id, { stops })` PATCH, 로컬 `currentCardOrder`로 화면 즉시 override (의도적으로 `invalidateQueries` 미호출 — 새 sequence로 재정렬 시 `currentSegmentIndex`가 다른 seg를 가리켜 `currentGroupIndex`가 "마지막 구간"으로 점프). 실패 시 롤백 + `showApiErrorToast`. `currentRouteIdRef`로 route 전환 race 가드. 신규 등록 경로의 stepGroup 순서 변경은 `SetupRoute` 헤더의 `ArrowLeftRight` swap 버튼 사용.
