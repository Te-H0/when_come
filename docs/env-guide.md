...
| `GYEONGGI_BUS_API_KEY` | 경기 버스 도착정보 + 노선조회 (GBIS) | [공공데이터포털](https://data.go.kr) | 도착정보·노선조회 데이터셋 각각 활용신청 필요 |
| `GYEONGGI_OPENAPI_KEY` | 경기도 정류소 현황 (캐싱 동기화용) | [경기도 OpenAPI 포털](https://openapi.gg.go.kr) | 별도 회원가입 필요. 호출 제한 없음. `sync-gbis-stations` 함수에서만 사용 |
