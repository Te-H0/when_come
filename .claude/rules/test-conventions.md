---
paths:
  - "when_come_fe/src/**/*.test.ts"
  - "when_come_fe/src/**/*.test.tsx"
  - "when_come_fe/src/**/*.spec.ts"
  - "when_come_fe/src/**/*.spec.tsx"
---
...
| 레이어 | 방식 | 도구 | 언제 |
|--------|------|------|------|
| 커스텀 훅 | 단위테스트 | `renderHook` | 복잡한 상태 로직 |
| 컴포넌트 | 통합테스트 | React Testing Library | 렌더링, 사용자 인터랙션 |
| 서비스(API) | 단위테스트 | msw + vitest | API 호출, 응답 변환 |
| 유틸 함수 | 단위테스트 | vitest | 순수 함수 (`transitColors` 등) |
...
MSW(Mock Service Worker)로 네트워크 레벨 모킹. fetch 직접 모킹 금지.
