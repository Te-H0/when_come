...
Deno.test(`arrival-info subway — 역명 끝의 "역" 접미사를 제거하여 외부 API에 요청한다`, async () => {
  let capturedUrl = ""
  await withEnv(ENV, () =>
    withMockFetch(async (url) => {
      capturedUrl = url
      ...
      // "역" 접미사가 제거된 "강남"의 인코딩(%EA%B0%95%EB%82%A8)이 포함되어야 함
      assertEquals(capturedUrl.includes(encodeURIComponent("강남")), true)
      // 원본 "강남역"의 인코딩이 포함되지 않아야 함
      assertEquals(capturedUrl.includes(encodeURIComponent("강남역")), false)
    })
  )
})
