...
supabaseTest("favorite-stops POST — 기존 favorite_stops 0건이면 display_order=0으로 저장된다", async () => {
    ...
    withMockFetch(
      async (url, init) => {
        ...
        if (url.includes("/rest/v1/favorite_stops")) {
          // maybeSingle() GET (max 조회) — 빈 응답 → null
        ...
        if (url.includes("favorite_stop_routes")) return mockDbInsertRoutes()
        return mockDbSelectFav()
      },
      async () => {
        await handler(makeReq("POST", "", {
...
supabaseTest("favorite-stops POST — 기존 favorite_stops 1건(display_order=5, 비연속)이면 신규는 display_order=6으로 저장된다", async () => {
  let capturedPayload: Record<string, unknown> | null = null
