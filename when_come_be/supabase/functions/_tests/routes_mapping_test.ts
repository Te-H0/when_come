...
// ─── 수정된 provider 결정 원칙 검증 ──────────────────────────────────────────
// 원칙: stop_routes.provider는 "실제 호출될 API"를 정확히 반영해야 한다.
// - stopProvider='gyeonggi'(GBIS 정류소 찾음) + 경기버스 → 'gyeonggi'
// - stopProvider='seoul'(서울 bbox, GBIS 못 찾음) + 경기버스 → 'seoul'
//   이유: Seoul BIS getStationByUid가 경기버스 도착정보도 같이 반환
//         'gyeonggi'로 저장하면 GyeonggiBusProvider.canHandle(gbisStationId=null) → false → 스킵

supabaseTest(
  "routes POST — busType===6 노선 + stopProvider='seoul'(서울 bbox) → provider='seoul'로 저장 (Seoul BIS가 처리)",
  async () => {
        ...
        async () => {
          const res = await handler(makePostRequest({
            ...
            stops: [{
              ...
              sequence: 1,
              stepGroup: 1,
              // lat/lng 없음 → stopProvider='seoul'(서울 fallback)
              stopRoutes: [
                // busType===6(경기버스) + stopProvider='seoul' → 'seoul' (Seoul BIS가 도착정보 제공)
                { odsayRouteId: "300000001", routeName: "경기버스", busType: 6 },
                // busType===6 + "2xxx" + stopProvider='seoul' → 'seoul'
          ...
          // stopProvider='seoul'이므로 경기버스도 'seoul'로 저장 (Seoul BIS 경유)
          const busType6Row1 = rows.find((r) => r.odsay_route_id === "300000001")
          assertEquals(busType6Row1?.provider, "seoul")
          const busType6Row2 = rows.find((r) => r.odsay_route_id === "200000001")
          assertEquals(busType6Row2?.provider, "seoul")
...
supabaseTest(
  "routes POST — stopProvider='seoul'(서울 bbox)이면 모든 노선이 'seoul'로 저장된다",
  async () => {
    let capturedStopRoutesBody: unknown = null
        ...
        async () => {
          const res = await handler(makePostRequest({
            ...
            stops: [{
              ...
              stepGroup: 1,
              // lat/lng 없음 → stopProvider='seoul'(서울 fallback)
              // 원칙: stopProvider='seoul'이면 모든 노선이 'seoul'로 저장
              // Seoul BIS getStationByUid가 경기버스 포함 모든 도착정보를 반환하기 때문
              stopRoutes: [
                { odsayRouteId: "100100643", routeName: "643" },  // 서울 노선
                { odsayRouteId: "213000006", routeName: "1" },    // 경기 노선 (2xxx) → 'seoul'로 다운그레이드
                { odsayRouteId: "300000001", routeName: "광역" }, // 비표준 → 'seoul'
              ],
            }],
          }))
          assertEquals(res.status, 201)
          ...
          const seoulRow = rows.find((r) => r.odsay_route_id === "100100643")
          assertEquals(seoulRow?.provider, "seoul")
          // stopProvider='seoul'이므로 경기 노선도 'seoul'로 저장
          const gyeonggiRow = rows.find((r) => r.odsay_route_id === "213000006")
          assertEquals(gyeonggiRow?.provider, "seoul")
          const nonStdRow = rows.find((r) => r.odsay_route_id === "300000001")
          assertEquals(nonStdRow?.provider, "seoul")
        },
      )
    )
  },
