 ...
 *
 * 정책: ADR-002 §4.1~§4.2
 * 카탈로그 문서: docs/api/error-codes.md
 *
 * 신규 코드 추가 절차:
 *  1. 도메인 union에 literal 추가 (이 파일)
 *  2. docs/api/error-codes.md 표에 행 추가
 *  3. BE 함수에서 throw 시 satisfies 사용
 *  4. 새 prefix 등장 시 ADR-002 §4.2 prefix 표 갱신
 *
 * 불변 원칙: 한 번 운영 배포된 코드는 rename 금지 — deprecated 처리 후 신규 추가.
 */
