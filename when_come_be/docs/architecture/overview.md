...
| favorite_stop_routes | 즐겨찾기 정류장의 노선 목록 (2026-05-09~). `stop_routes`와 동일 컬럼 셋 (odsay_route_id, route_name, bus_type, st_id, bus_route_id, station_ord, station_name, gbis_route_id, gbis_sta_order, provider). `favorite_stops(id)` cascade delete. RLS: 부모 favorite_stops.user_id 기반. |
